...
describe("Deposit 页（T7）", () => {
  it("渲染余额卡 + 充值/提现按钮 + 逐笔锁仓说明", () => {
    ...
    expect(screen.getByText(/locked independently for 30 days/)).toBeInTheDocument();
  });
  ...
  it("DEP-01: 充值抽屉只含 4 个档位按钮（10/20/50/100），各标注得卡张数", () => {
    ...
    expect(screen.getByText(/→ Get 1 unlimited traffic cards/)).toBeInTheDocument();
    expect(screen.getByText(/→ Get 10 unlimited traffic cards/)).toBeInTheDocument();
  });
  ...
  it("DEP-03: 选中档位 > 钱包 USDT 余额 → 拦截（错误文案 + 存入禁用）", () => {
    ...
    expect(screen.getByText(/Exceeds wallet USDT balance/)).toBeInTheDocument();
    expect(screen.getByTestId("two-step")).toBeDisabled();
  });
  ...
  it("WDR-03: 到期笔的「取回」按钮可点 → 调 withdraw(index)；锁定笔禁用", () => {
    ...
    render(<Deposit />);
    fireEvent.click(screen.getByText("Withdraw").closest("button")!);
    // 逐笔取回按钮限定在 tranche-row 内（与页面顶部「Withdraw」主按钮文案同为 Withdraw，故按 slot 定位）。
    const withdrawButtons = Array.from(
      document.querySelectorAll<HTMLButtonElement>('[data-slot="tranche-row"] button'),
    );
  ...
  it("WDR-04: 已取回笔显示「已取回」且无取回按钮", () => {
    ...
    // 已取回笔无逐笔取回按钮（tranche-row 内无 button）。
    expect(
      document.querySelectorAll('[data-slot="tranche-row"] button'),
    ).toHaveLength(0);
  });
